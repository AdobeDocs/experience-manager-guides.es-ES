...
El complemento Oxygen para Adobe Experience Manager Guides \(más adelante denominado complemento Oxygen para AEM Guides en la guía\) le permite conectar el Oxygen XML Author con el repositorio Adobe Experience Manager \(AEM\) para crear y administrar contenido. Puede utilizar el complemento para examinar, buscar y abrir archivos, desproteger y proteger archivos, cargar carpetas y archivos en el repositorio de AEM. El panel AEM Guides de la aplicación de escritorio le permite marcar las carpetas deseadas \(del repositorio de AEM\) a la lista de carpetas favoritas para acceder rápidamente a ellas. Además, puede instalar un paquete en la interfaz web de AEM y abrir los archivos DITA en Oxygen XML Author directamente desde la interfaz web de AEM.

## Descargar e instalar {#id1826M0L0PUI}

El complemento Oxygen para AEM Guides está disponible a través de su portal de distribución de software de Adobe. Busque &quot;oxígeno&quot; en la ficha Experience Manager y, a continuación, descargue el programa de instalación del complemento desde su [Portal de distribución de software de Adobe](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html).
...
### Instale el paquete para habilitar la función de edición de documentos desde la interfaz web de AEM {#id182CE0Q0TY4}

Como autor, puede abrir y editar sus mapas o temas DITA en Oxygen XML Author directamente desde la interfaz web de AEM. Para habilitar esta función en la interfaz web de AEM, el administrador de AEM debe instalar un paquete en la instancia de creación de AEM.
...
1. Inicie sesión en la instancia de AEM *\(como administrador\)* y vaya al Administrador de paquetes de CRX. La URL predeterminada para acceder al administrador de paquetes es
...
1. En el cuadro de diálogo Cargar paquete, vaya al archivo del paquete Oxygen que descargó en el paso 1 y haga clic en Aceptar.
   ...
   El paquete se cargará en la instancia de AEM.
...
1. Seleccione un archivo DITA en la carpeta de recursos.
   ...
   La opción **Editar en oxígeno** está disponible en la barra de herramientas. Para obtener más información sobre el uso de esta opción, consulte [Abrir tema de DITA en Oxygen XML Author desde la interfaz web de AEM](#id182CE0I905Z).
...
- **Configuración general**: Configuración de conexión para el complemento, como la URL del servidor de AEM, los detalles de inicio de sesión, etc.
...
1. Especifique los siguientes detalles:
   - **URL del servidor**: URL del servidor de AEM, por ejemplo:
     ...
     En la URL anterior, especifique el nombre de host y el puerto del servidor donde se implementa el servidor de AEM.

     >[!IMPORTANT]
     >
     >Si el servidor AEM está implementado en el puerto 80 o 443, no es necesario especificarlo en la dirección URL.
   ...
   - **Autenticación:** Elija entre **Básica \(Nombre de usuario/Contraseña\)** o **Autenticación web**. Si selecciona la autenticación **Basic**, debe introducir el **Nombre de usuario** y la **Contraseña** en el cuadro de diálogo Preferencias.
     ...
     Si selecciona Autenticación web, aparecerá la pantalla de inicio de sesión de AEM. Escriba sus credenciales de inicio de sesión y haga clic en el botón **Iniciar sesión**. Cuando el inicio de sesión se realiza correctamente, la pantalla Inicio de sesión de AEM se cierra y el panel AEM Guides muestra la lista de archivos del servidor de AEM.

   - **Tiempo de espera de conexión**: especifique el tiempo en segundos que el cliente esperará una respuesta del servidor de AEM. Si no se recibe ninguna respuesta del servidor en el tiempo especificado, se finaliza la solicitud. El valor predeterminado es 20 segundos.

   - **Carpeta local**: ubicación en el equipo local donde se almacenan los archivos del repositorio de AEM después de la desprotección. Si especifica una ubicación que no existe en la unidad, el complemento crea esa ubicación.
...
Es necesario configurar las preferencias en Oxygen XML Author para utilizar el atributo de generación de perfiles asociado a los temas DITA en el repositorio de AEM. También debe configurar la preferencia para mostrar los nombres de archivo en lugar de los GUID en las referencias cruzadas.
...
La configuración de la extensión de mapa DITA es necesaria para permitir la apertura de archivos de mapa en Oxygen XML Author directamente desde la interfaz web de AEM. Estas configuraciones son similares a las configuraciones de generación de perfiles de atributos realizadas en el procedimiento anterior.
...
1. En la ficha **Extensiones**, realice los cambios siguientes:
   - Haga clic en **Elegir** junto al paquete de **Extensiones** y seleccione `com.adobe.o2.framework.extn.LinkResolverDITAMapExtensionBundle` en la lista **Clase**. Haga clic en **OK**.
...
**B**\) Muestra la carpeta Favoritos. De forma predeterminada, está vacío. Puede agregar carpetas del repositorio de AEM como favoritas; a continuación, las carpetas favoritas se muestran aquí.

**C**\) La carpeta DAM muestra el repositorio de AEM. Puede expandir y contraer la vista de carpetas.

**D**\) El icono \(engranaje\) de Configuración con las siguientes opciones:

- **Conectar**: seleccione esta opción para conectarse al servidor de AEM. La opción está desactivada cuando Oxygen XML Author está conectado al servidor de AEM.
- **Actualizar**: seleccione esta opción para obtener el estado más reciente de los archivos y carpetas del repositorio de AEM.
...
- **Cerrar sesión**: seleccione esta opción para cerrar la conexión con el servidor de AEM. Esta opción solo está disponible si utiliza el modo Autenticación web.
...
- **Agregar a Favoritos**: Agrega una carpeta a la carpeta *Favoritos* del panel AEM Guides. Se recomienda agregar la carpeta de trabajo aquí, lo que facilita la sincronización de archivos y el estado del archivo desde AEM.
...
Una vez que se haya conectado al repositorio de AEM, puede abrir los archivos para editarlos en Oxygen XML Author. Realice los siguientes pasos para abrir un archivo para editarlo en Oxygen XML Author:
...
Al retirar un archivo, se almacena localmente en el sistema y se bloquea para su edición en el repositorio de AEM. Realice los siguientes pasos para desproteger un archivo:
...
1. Seleccione una de las siguientes opciones:
   - **Desproteger:** desprotege un archivo del repositorio de AEM y lo pone a disposición para su edición.
...
>[!NOTE]
>
>Si el archivo desprotegido se elimina o se mueve a cualquier otra carpeta de AEM, aparecerá un mensaje de error al proteger el archivo. Asegúrese de que el archivo desprotegido no se mueve ni se elimina mediante la interfaz web de AEM.

### Proteger un archivo {#id182CF0J0FHS}

Al registrar un archivo, la copia local del sistema se almacena en el repositorio de AEM y se elimina el bloqueo del archivo. Siga estos pasos para proteger un archivo:
...
1. Elija entre las dos opciones siguientes:
   ...
   - **Proteger**: protege el archivo seleccionado del sistema local en el repositorio de AEM.
   ...
   Si se extrae un archivo dependiente en AEM, la opción Registrar con dependientes no mostrará el archivo dependiente en el cuadro de diálogo Registrar. Para obtener una lista de archivos dependientes desprotegidos en AEM, debe actualizar la carpeta.

   Del mismo modo, si ha protegido un archivo dependiente a través de AEM, la lista de archivos no se actualiza en Oxygen Author hasta que realice una actualización de carpeta y actualice los archivos desprotegidos. Si realiza un registro con dependientes con algunos archivos registrados a través de AEM, obtendrá un error que enumera los archivos que no se pudieron registrar.
...
1. \(Opcional\) En el cuadro de diálogo **Registrar** o **Registrar con dependientes**, agregue un comentario en el cuadro de texto **Comentarios de la versión**.
   ...
   >[!NOTE]
   >
   >Este comentario se muestra en el historial de versiones de AEM del archivo.
...
Sin embargo, no se puede agregar la misma etiqueta a las distintas versiones de un tema. Si agrega una etiqueta que ya ha agregado a una versión anterior, se agrega a la versión más reciente y se elimina de la versión anterior.
   ...
   >[!NOTE]
   > 
   > Estas etiquetas se muestran en el historial de versiones de AEM del archivo.
...
1. Seleccione una de las siguientes opciones:
   - **Cargar archivo\(s\)**: seleccione esta opción para cargar uno o varios archivos en la carpeta seleccionada en el repositorio de AEM. En el cuadro de diálogo Seleccionar archivos \(s\) para cargar, seleccione los archivos y haga clic en **Abrir**.
   ...
   - **Cargar carpeta**: seleccione esta opción para cargar una carpeta en el repositorio de AEM. En el diálogo Elegir, seleccione la carpeta y haga clic en **Elegir**.
...
- Cuando retire contenido del repositorio de AEM y realice cambios en el sistema local, asegúrese de que el nombre del archivo no cambie en el momento de cargarlo.
...
- Puede quitar una carpeta de favoritos de las siguientes maneras:
   ...
   - Haga clic con el botón derecho en una carpeta del repositorio de AEM bajo la carpeta **DAM** que ya se ha agregado como favorita y seleccione **Quitar de Favoritos**.
...
Puede buscar temas en el repositorio de AEM mediante la barra de búsqueda del panel AEM Guides. Puede buscar en toda la carpeta DAM o seleccionar una carpeta y, a continuación, buscar un tema en esa carpeta. El resultado de la búsqueda muestra los temas cuyo texto coincide con la consulta de búsqueda.

Siga estos pasos para buscar temas:

1. Seleccione una carpeta del repositorio de AEM en la que desee buscar un tema.
...
1. Para volver a la vista Repositorio de AEM, siga uno de estos procedimientos:
   - Para ver la vista del repositorio de AEM sin borrar los resultados de la búsqueda, haz clic en la pestaña **Examinar**.
   - Para borrar los resultados de búsqueda y ver el repositorio de AEM, haga clic en el icono Eliminar búsqueda.

## Abrir tema DITA en Oxygen XML Author desde la interfaz web de AEM {#id182CE0I905Z}

Puede abrir y editar el tema DITA en Oxygen XML Author desde la interfaz web de AEM. Debe instalar un paquete en AEM para activar esta opción. Para obtener más información acerca de la instalación del paquete, consulte [Instalar el paquete para habilitar la característica de edición de documentos desde la interfaz web de AEM](#id182CE0Q0TY4).

>[!NOTE]
>
>Se puede acceder a la opción **Editar en oxígeno** desde varios lugares de AEM: cuando se selecciona un tema, cuando se previsualiza un tema o desde la pestaña Temas e informes de la consola de mapas DITA. Si selecciona varios temas, la opción no estará visible en la barra de herramientas.
...
1. Seleccione Oxygen XML Author *&lt;version\>* en el cuadro de mensaje **Iniciar aplicación**. Puede seleccionar la opción **Recordar mi elección de vínculos de AEM** para guardar su preferencia.
...
1. Consulte el tema en el complemento Oxígeno para AEM Guides.
   ...
   >[!NOTE]
   >
   >Asegúrese de proteger el tema con el complemento Oxygen para AEM Guides. Si protege desde la interfaz web de AEM, los cambios que realice en Oxygen XML Author no se guardarán en la versión protegida del tema.
...
- El elemento `<xref>` se agrega para todas las demás referencias como topic, map, DITAVAL, PDF, ZIP y XML.
...
Problema: Cuando elige abrir un archivo para editarlo en Oxygen XML Author desde el servidor de AEM, el archivo se abre para editarlo en Oxygen XML Author. Sin embargo, el panel AEM Guides no muestra la ubicación del archivo en el árbol de navegación.

Solución: este problema se ha observado en escenarios en los que la ruta de archivo contiene /content/dam dos veces. De forma predeterminada, todos los recursos de AEM se almacenan en la carpeta /content/dam. Si carga o crea una estructura de carpetas que también contiene /content/dam, se observa este problema. Puede realizar todas las operaciones normales con estos archivos, pero su ubicación en el árbol de navegación no se muestra de forma predeterminada. Para acceder a dicho archivo en el árbol de navegación, debe examinar manualmente la ubicación del archivo. Tenga en cuenta que en el árbol de navegación, la ruta duplicada /content/dam se reemplaza por /content/assets.
