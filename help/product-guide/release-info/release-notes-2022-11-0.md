...
1. Actualice la propiedad `<dox.version>` en el archivo `/dox/dox.installer/pom.xml` de su código Git de Cloud Services a 2022.11.198.
1. Confirme los cambios y ejecute la canalización de Cloud Services para actualizar a la versión de noviembre de AEM Guides as a Cloud Service.
...
* La API devolverá un jobId. Para comprobar el estado del trabajo, puede enviar una solicitud GET con el ID del trabajo al mismo punto final: `http://<server:port>/bin/guides/map-find/indexing?jobId={jobId}`
...
Esta sección enumera la matriz de compatibilidad para las aplicaciones de software compatibles con la versión de noviembre de 2022 de AEM Guides as a Cloud Service.
...
Si el archivo seleccionado está desprotegido, no podrá eliminarlo y aparecerá un mensaje de error. Si el archivo seleccionado se agrega a una colección de favoritos o se hace referencia a él desde cualquier otro archivo, las guías de AEM comprueban la confirmación y le ofrecen la opción de eliminarlo a la fuerza. Si elimina un tema al que se hace referencia y ha abierto el archivo que contiene referencias para su edición, se mostrará el vínculo roto para el archivo al que se hace referencia.
...
* PDF nativo | `conkeyref` no se resuelve en la salida de PDF generada. (10564)
* PDF nativo | Se producen problemas al acceder a los metadatos de un mapa en la salida de PDF. (10556)
...
* La publicación de PDF falla si creamos un ajuste preestablecido mediante la duplicación de un ajuste preestablecido existente. (10584)
* El botón **Ver registro** no funciona cuando la generación de PDF falla para un ajuste preestablecido. (10576)
...
* Al usar una exportación de PDF nativa, los `<indexterm>` anidados no se anidan en el índice. (10521)
...
* Los archivos SVG no se muestran en el modo de vista previa. (10010)
